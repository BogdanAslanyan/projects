...
void DepositCalc::slotCalc() {
  ...
  dd.depositAmount = dsDepAmount->value();
  dd.periodOfPlacement = static_cast<double>(sPeriodOfPlacement->value());
  dd.interestRate = dsInterestRate->value();
  dd.taxRate = dsTaxRate->value();
  dd.paymentsType = cmPerOfPay->currentIndex();
  dd.interestCapitalization = cbCapital->isChecked() ? 1 : 0;
  dd.currentMoth = sbStartDateMonth->value();
  dd.currentYear = sbStartDateYear->value();
  dd.currentDay = sbStartDateDay->value();
  dd.addCount = twAdd->rowCount();

  for (int i = 0; i < twAdd->rowCount(); i++) {
    dd.replenishmentList[i].term =
        qobject_cast<QComboBox *>(twAdd->cellWidget(i, 0))->currentIndex();
    dd.replenishmentList[i].money =
        qobject_cast<QDoubleSpinBox *>(twAdd->cellWidget(i, 1))->value();
  }
  dd.remCount = twRem->rowCount();
  for (int i = 0; i < twRem->rowCount(); i++) {
    dd.listPartialWithdrawals[i].term =
        qobject_cast<QComboBox *>(twRem->cellWidget(i, 0))->currentIndex();
    dd.listPartialWithdrawals[i].money =
        qobject_cast<QDoubleSpinBox *>(twRem->cellWidget(i, 1))->value();
  }

  s21::Controller controller(&dd);

  controller.callDeposit();

  sumInterest->setText("Начисленные проценты: " +
                       QString::number(dd.sumInterest, 'f', 2));
  sumEndTerm->setText("Сумма на вкладе к концу срока: " +
                      QString::number(dd.sumEndTerm, 'f', 2));
  sumTax->setText("Cумма налога: " + QString::number(dd.sumTax, 'f', 2));
