...
3. Провел те же тесты, что в предыдущем таске, результаты на графах.
...
1. Сделал проброс портов 81:81, установил nginx. 

2. Редактировал nginx.conf, в котором обозначил, что сервер будет слушать порт 81, а также указал, что на страничке 127.0.0.1:81 будем отражать страницу nginx по умолчанию, а на 127.0.0.1:81/metrics - метрики. Далее именно с этой страницы в графане мы сможем отразить собранные метрики. Перезапустил nginx.
...
3. Написал скрипт, который собирает метрики. Метрики передаются в html-файл и записаны определенным образом -- так же, как в prometheus/metrics:
...
4. В prometheus.yml указал, что метрики будут собираться с 127.0.0.1:81 каждые 3 секунды:
...
5. В grafana создал дашборд и выбрал свои метрики: 
