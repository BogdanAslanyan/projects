...
template <typename value_type>
list<value_type>::list() : head(nullptr), tail(nullptr), sizeOfList(0) {}
...
typename list<value_type>::reference list<value_type>::operator[](
    const int& index) {
  if (index < 0 || index >= sizeOfList) throw std::out_of_range("Out of list");
  Node<value_type>* cur = head;
...
list<value_type>::list(list&& l)
    : head(l.head), tail(l.tail), sizeOfList(l.sizeOfList) {
  l.sizeOfList = 0;
  l.head = nullptr;
  l.tail = nullptr;
}
